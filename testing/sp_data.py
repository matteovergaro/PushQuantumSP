...
from data.glb_reader_small import create_problem_from_glb
...
class SPData:
    ...
    def __init__(
        ...
        self.are_edges_weighted = weightdist
        # Variable that contains the weights of the edges
    ...
    @classmethod
    def gen_problem(cls, num_cols, version, rad_max=2.5, hor_basic_distance=1, vert_basic_dist=2, weightdist=True):
        data_params = {
            "vert_ang_max_deg": 30,
            "vert_ang_min_deg": -70,
            "halber_oeffnungswinkel_deg": 180,
        ...
        cls.are_edges_weighted = weightdist
        if version == 1:
            return cls._gen_problem(num_cols, [0], 0, (num_cols - 1) * hor_basic_distance, 2.5, 0, -10, 0,
                                    (num_cols - 1) * hor_basic_distance, vert_basic_dist, vert_basic_dist, 1, num_cols,
                                    **data_params)
        elif version == 2:
            return cls._gen_problem(num_cols, [0], 0, (num_cols - 1) * hor_basic_distance, 2.5, 0, -10, 0,
                                    (num_cols - 1) * hor_basic_distance, 0.5 * vert_basic_dist, vert_basic_dist, 2,
                                    num_cols, **data_params)
        elif version == 3:
            return cls._gen_problem(num_cols, [0, 2 * vert_basic_dist], 0, (num_cols - 1) * hor_basic_distance, 2.5, 0,
                                    -10, 0, (num_cols - 1) * hor_basic_distance, 0.5 * vert_basic_dist,
                                    1.5 * vert_basic_dist, 3, num_cols, **data_params)
        elif version == 4:
            return cls._gen_problem(num_cols, [0, vert_basic_dist, 2 * vert_basic_dist], 0,
                                    (num_cols - 1) * hor_basic_distance, 2.5, 0, -10, 0,
                                    (num_cols - 1) * hor_basic_distance, 0.25 * vert_basic_dist, 1.75 * vert_basic_dist,
                                    4, num_cols, **data_params)
        else:
            print("Version can be ońly 1,2,3 or 4")
    ...
    @classmethod
    def _gen_problem(cls, l_number_per_row, l_y, l_xmin, l_xmax, l_height, l_yaw_deg, l_pitch_deg, s_xmin, s_xmax,
                     s_ymin, s_ymax, s_rows, s_cols, **data_params):

        problem_dict = cls.problem_generator(l_number_per_row, l_y, l_xmin, l_xmax, l_height, l_yaw_deg, l_pitch_deg,
                                             s_xmin, s_xmax, s_ymin, s_ymax, s_rows, s_cols)
        new_class = cls(**data_params)
        new_class.create_graph_from_dict(problem_dict)
        return new_class

    @classmethod
    def problem_generator(cls, l_number_per_row, l_y, l_xmin, l_xmax, l_height, l_yaw_deg, l_pitch_deg, s_xmin, s_xmax,
                          s_ymin, s_ymax, s_rows, s_cols):

        lid = []
        for i in l_y:
            lid.extend(cls.create_horizontal_lidar_points(l_number_per_row, i, l_xmin, l_xmax, l_height, l_yaw_deg,
                                                          l_pitch_deg))
    ...
    def weight_calculation(self, sx, sy, lx, ly):
        dist = math.sqrt((sx - lx) ** 2 + (sy - ly) ** 2)
        weight = self.rad_max - dist
        if weight < 0:
            weight = 0
        return weight
    ...
    def create_connections(self):
        ...
        for s in self.listStreetPoints3D:
            ...
            for l in self.listLidar3D:
                line = (l, s)
                if (self._in_range(l, s, self.rad_max, self.vert_ang_max_deg, self.vert_ang_min_deg,
                                   self.halber_oeffnungswinkel_deg)):
                    inters = 0
                    ...
                    if inters == 0 and self.are_edges_weighted:
                        ...
                        # edge weight as weight = rad_max - distance, if distance is greater than rad_max the
                        # weight is set to 0
                        weight = self.weight_calculation(s[0], s[1], l[0], l[1])
                        self.G.add_edge((s[0], s[1], s[2]), (l[0], l[1], l[2], l[3], l[4]), weight=weight)
                        key = frozenset([s, l])
                        self.custom_edge_weights[key] = weight
                        edgecount += 1
                        nc = 0
                    if inters == 0 and (not self.are_edges_weighted):
    ...
    def __generateGraph3D(self):
        ...
        for w in self.schemeGraph['wall']:
            ...
            if w[3] > 0:
                ...
                difflidar = [diff[0] * (1 - 2 * self.lidarwall_offset_m / length),
                             diff[1] * (1 - 2 * self.lidarwall_offset_m / length)]
                ...
                difflidarstart = [w[0][0] + self.lidarwall_offset_m / length * diff[0] + perpendicular_offset[0],
                                  w[0][1] + self.lidarwall_offset_m / length * diff[1] + perpendicular_offset[1]]
                if numlid > 1:
    ...
    def _in_range(cls, l, s, maxcoverage_meter, vert_ang_max_deg, vert_ang_min_deg, halber_oeffnungswinkel_deg):
        ...
        if vert_angle > vert_ang_max_deg / 180 * math.pi or vert_angle < vert_ang_min_deg / 180 * math.pi:
            #print(
            #    f"vertical is too much, vert angle {vert_angle} while ver max {vert_ang_max_deg / 180 * math.pi} and vert min {vert_ang_min_deg / 180 * math.pi}")
            return 0
